 ...
 * File: BleManager.cpp
 * Version: 1.47.1
 * Изменение: Обновлена запись телеметрии с учетом переименованного поля gpsState.
 ...
 */
 ...
 // ИЗМЕНЕНИЕ 1.47.1: Используется gpsState (0, 1 или 2)
 void BleManager::sendMyStatus(uint8_t gpsState, uint8_t satellites, uint8_t batteryPercent, uint16_t batteryVoltage) {
      if (!_isConnected) return;
 
      BleEvtMyStatus packet;
      packet.opCode = EVT_MY_STATUS;
      packet.gpsState = gpsState; // НОВОЕ

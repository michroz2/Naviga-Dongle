 ...
 * File: main.cpp
 * Version: 1.43.4
 * Изменение: Рефакторинг (Шаг 4) - Изоляция логики адаптивной отправки
 * координат (Smart TX). Description: Главный файл оркестратора.
 */
...
uint8_t myNodeType =
    NODE_RELAY; // Роль узла по умолчанию (перезапишется из настроек)
...
float lastScaleLat =
    0.0f; // Широта, при которой последний раз был рассчитан масштаб
...
uint32_t calculateRelayJitter(uint8_t myRole, uint8_t senderRole, float snr) {
  ...
  // VIP-Маршрутизация: Если мы ретранслируем пакет ТРЕКЕРА, даем ему зеленый
  // свет
...
void handleCollision() {
  ...
  } while (nodeDB.getNode(myNodeId) !=
           nullptr); // Проверяем, свободен ли он в базе

  nodeDB.addNode(myNodeId);

  LOG_WARN("COLLISION", "ID %d is taken! Switched to new ID: %d", oldId,
           myNodeId);
...
void scanNetwork(bool isWarmStart) {
  if (isWarmStart) {
    LOG_INFO("SYS", "Warm Start: Silent listening for %d ms...",
             networkScanDuration);
  } else {
          ...
          if (router.isValidPacket(rxHeader.getType(), payloadLen)) {
            if (!router.isDuplicate(rxHeader.senderId, rxHeader.msgSeq)) {
              packetManager.processPacket(
                  rxHeader, rxBuffer + sizeof(NavigaHeader), payloadLen);
            }
          }
        }
      }
  ...
  display.showStatus("Scan Complete", "My ID:", String(myNodeId),
                     "Starting...");
  delay(2000);
}

// ==========================================================
// ПРОЦЕДУРЫ ОРКЕСТРАТОРА (main.cpp)
// ==========================================================
...
void processBleCommands() {
  ...
  if (bleManager.hasNewIdentity) {
    ...
    strncpy(settingsManager.settings.nodeName, bleManager.newIdentity.myName,
            sizeof(settingsManager.settings.nodeName) - 1);
    settingsManager.settings
        .nodeName[sizeof(settingsManager.settings.nodeName) - 1] = '\0';
    settingsManager.save();
...
void processBackgroundTasks(uint32_t currentMillis, float currentSpeed,
                            bool isFastTracker) {
  if (gps.isValid() &&
...
void updateGeodata(bool isFastTracker) {
  if (gps.isValid()) {
    ...
    if (!isLonScaleSet || abs(currentLat - lastScaleLat) > 1.0f) {
      packer.updateLonScale(currentLat);
    ...
    if (!isFastTracker) {
      for (int i = 1; i < 255; i++) {
        const NodeRecord *node = nodeDB.getNode(i);
        if (node != nullptr && node->isActive) {
          if (node->packedCoords != 0 && node->lat == 0.0f &&
              node->lon == 0.0f) {
            float unpLat, unpLon;
            packer.unpack(node->packedCoords, gps.getLat(), gps.getLon(),
                          unpLat, unpLon);
            nodeDB.updateNodeCoords(i, unpLat, unpLon, node->packedCoords,
                                    false);
          }
...
void updateDisplay() {
  ...
  int targetQuality =
      isTargetValid ? getConnectionQuality(targetNode->nodeId) : 0;
...
// === ОБРАБОТКА ВХОДЯЩИХ ПАКЕТОВ LORA ===
  ...
  if (receivedFlag) {
    ...
    if (len > 0) {
      ...
      if (state == RADIOLIB_ERR_NONE) {
        ...
        if (len >= sizeof(NavigaHeader)) {
          NavigaHeader rxHeader;
          memcpy(&rxHeader, rxBuffer,
                 sizeof(NavigaHeader)); // Извлекаем заголовок
          size_t payloadLen = len - sizeof(NavigaHeader);
          ...
          if (isOwnEcho) {
            ...
            if (updatedNode != nullptr) {
              ...
              strncpy(update.nodeName, updatedNode->nodeName,
                      sizeof(update.nodeName) - 1);
              update.nodeName[sizeof(update.nodeName) - 1] = '\0';
            ...
            if (isNewNode && rxHeader.senderId != myNodeId) {
              ...
              lastHeartbeatTime =
                  currentMillis - HEARTBEAT_INTERVAL_MS + jitterMs;
              LOG_INFO("SYS",
            ...
            if (router.shouldRetransmit(rxHeader, nodeDB, myNodeType,
              ...
              uint32_t calculatedJitter =
                  calculateRelayJitter(myNodeType, senderRole, currentSNR);
...
// Отключаем встроенный LoRa, если это плата T-Beam v1.1
  ...
  // Искусственно стартим восстановленные узлы, чтобы при Warm Start они были
  // "серыми"
  ...
  // Если устройство является стационарным ретранслятором, задаем ему
...
void loop() {
  ...
  // 1. Команды от смартфона по Bluetooth
  processBleCommands();
  ...
  // 2. Фоновые задачи (Топология, GC)
  processBackgroundTasks(currentMillis, currentSpeed, isFastTracker);

  // 3. Периодическая очистка устаревших узлов (Garbage Collector)
  if (currentMillis - lastCleanupTime > CLEANUP_INTERVAL_MS) {
    nodeDB.cleanup(myNodeId);
    lastCleanupTime = currentMillis;
  }

  // 4. Регулярная отправка "живого" пинга (NodeInfo) - Heartbeat
  if (currentMillis - lastHeartbeatTime > HEARTBEAT_INTERVAL_MS) {
    sendHeartbeat();
    lastHeartbeatTime = currentMillis;
  }

  // 5. Обновляем данные с GPS-чипа
  gps.update();

  // 6. Регулярная отправка Телеметрии по Bluetooth
  if (currentMillis - lastTelemetryTime >= TELEMETRY_INTERVAL_MS) {
    sendTelemetry();
    lastTelemetryTime = currentMillis;
  }

  // 7. Обработка входящих пакетов LoRa (RX)
  processIncomingPackets();

  // 8. Адаптивная отправка собственных координат (Smart TX)
  smartPosManager.process();

  // 9. Обработка очереди на передачу в радиоэфир (Освобождение CSMA/CA)
  txManager.processQueue();

  // 10. Ежесекундные расчеты и отрисовка UI
  if (currentMillis - lastGpsLogTime >= gpsUpdateInterval) {
    lastGpsLogTime = currentMillis;
    display.toggleLed();
    updateGeodata(isFastTracker);
    updateDisplay();
  }
} // MAIN.CPP
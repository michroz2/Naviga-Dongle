 ...
 * Version: 1.47.0
 * Изменение: Реализованы методы для работы со статической опорной точкой в NVS.
 ...
 */
 ...
 // НОВОЕ 1.47.0: Сохранение статических координат для устройств без GPS
 void SettingsManager::saveStaticCoordinates(float lat, float lon) {
     _preferences.begin(PREFS_NAMESPACE, false);
     _preferences.putFloat(PREFS_STAT_LAT_KEY, lat);
     _preferences.putFloat(PREFS_STAT_LON_KEY, lon);
     _preferences.end();
     LOG_INFO("SYS", "Permanent NVS Anchor saved: Lat=%.6f, Lon=%.6f", lat, lon);
 }
 
 // НОВОЕ 1.47.0: Чтение статических координат из NVS
 bool SettingsManager::loadStaticCoordinates(float &lat, float &lon) {
     _preferences.begin(PREFS_NAMESPACE, true);
     if (_preferences.isKey(PREFS_STAT_LAT_KEY) && _preferences.isKey(PREFS_STAT_LON_KEY)) {
         lat = _preferences.getFloat(PREFS_STAT_LAT_KEY, 0.0f);
         lon = _preferences.getFloat(PREFS_STAT_LON_KEY, 0.0f);
         _preferences.end();
